// VENDORED from snapcn (https://snapcn.dev) — registry/r/cursor-track.json
// Source repo: github.com/snapcndev/snapcn | License: MIT | Fetched: 2026-09-01
// Local intake changes:
//   - removed snap-cn-ui theme system (useSnapCnTheme/SnapCnTheme): default colors
//     now come from scene-kit tokens (ink/white outline/brand ring); theme/mode props dropped
//   - withAlpha() inlined (hex -> rgba, logic copied from snapcn core/color.ts)
//   - dropped "use client" (not needed inside a Remotion bundle)
// Everything else (waypoint model, easing, click ring, arrow SVG) is upstream code, unchanged.
...
export interface CursorTrackProps {
   ...
   */
  outlineColor?: string;
  /** The click ring. Defaults to brand blue. */
  ringColor?: string;
...
export function CursorTrack({
  ...
  const ring = ringColor ?? COLOR.brand;
