---
name: ppt
description: 用 python-pptx 代码生成专业 PPT / 幻灯片 / 演示文稿（deck / slides / pptx）。用户说「做个 PPT / 幻灯片 / 演示文稿 / 项目介绍 deck / 把项目做成 slides」时使用；改现有 pptx 同样适用。用户说做介绍视频时不要只开本 skill，走 workflows/explainer-video.md，本 skill 只负责其中的 deck 取材。
user-invocable: true
---
...
**先答受众三问再动笔**：给谁看（外行 / 内行 / 客户）？观众对项目已知道什么？看完要能复述哪三句话？答案写在文案稿头部，之后所有取舍用它裁决。
...
**大纲先于逐页**：挖完先写叙事大纲——一条主线（整份 PPT 只论证一件事）+ 开篇三答（是什么 / 给谁 / 解决什么）+ 每页在论证链里的角色（承接上页什么、铺垫下页什么）。概念先解释后使用，禁前向引用；连续两页互不承接 = 重排。逐页文案稿（标题 + 要点 + 口播稿；口播稿生成时进 `notes()` 演讲者备注，是介绍成片工作流的文案源与现场讲稿）挂在大纲下写，标题要参与递进，不是各自成立的口号。
...
2. **数字带语境**：数值 + 口径 + 构成或对比，且是观众在乎的数字；挖不到语境的数字不当主视觉
...
| 维度 | 规则 |
|---|---|
| 色板 | 从项目设计语言提取（README 视觉节 / 站点主色 / 品牌 VI）；无则 `use_theme('warm' / 'tech' / 'forest')`。**7+1 封闭系统（7 角色 + 纸白），中途禁加色**；小字对底色 ≥4.5:1 |
...
# 增长叙事（morph 真补间，占连续两页，auto 自动跳过；生效验证 EntryEffect==3954；2019+，WPS 降级 fade）：
...
**② 视觉盲看**：pymupdf 渲 PNG（`fitz` + `get_pixmap(dpi=100)`）→ 交**无生成上下文**的子代理两问：其一逐页查四项（溢出 / 乱码 / 对齐 / 对比度）；其二**观众复述测试**——只给它前 1/3 页，问「这是什么、给谁用、解决什么问题」，答不出 = Step 1 受众或开篇没做对，打回重写文案再生成。盲看提示里不要预设豁免（不说『这是故意的』）：被页缘裁切的元素、超大低对比装饰字这类可疑形态，同样交新鲜眼光当疑点报。发现问题改脚本重跑，**禁手改 pptx**。

读图坑：路径含反斜杠会解析失败，先复制到 `C:\pc\` 类短路径；复查修复效果必须换新文件名（同路径图片会被缓存返回旧图）。
