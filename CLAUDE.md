...
自媒体内容工厂：项目 README → 介绍 PPT → 逐页动效 → 视频成片的一条流水线。不承载业务代码，只沉淀内容生产能力（Skills / Workflows / Assets）。当前 `ppt`、`humanizer` 两个 skill 已上线；`vibe-motion`（逐页动效）、`deck-to-video`（Remotion 合成）规划中。
