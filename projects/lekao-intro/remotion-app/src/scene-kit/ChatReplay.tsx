...
const ShellChat: React.FC<{ messages: ChatMessage[]; delay: number; width: number; title: string }> = ({
  messages,
  delay,
  width,
  title,
...
export const ChatReplay: React.FC<{
  ...
  shellTitle?: string; // 聊天头名称（shell 模式）
  style?: React.CSSProperties;
}> = ({ messages, width = 560, delay = 0, shell = false, shellTitle = "AI 助手", style }) => {
  const frame = useCurrentFrame();
  const { fps } = useVideoConfig();
  const t = Math.max(0, frame / fps - delay);
  const appear = appearTimes(messages);
  ...
  if (shell) {
    return <ShellChat messages={messages} delay={delay} width={width || 340} title={shellTitle} />;
  }
